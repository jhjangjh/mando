#include <pedestrian_lidar.hpp>

PedestrianLidar::PedestrianLidar(ros::NodeHandle &nh_){
...
void PedestrianLidar::ProcessINI(){
    if (v_ini_parser_.IsFileUpdated()){
        // v_ini_parser_.ParseConfig("tunnel_lidar", "xxx",
        //                             tunnel_lidar_params_.xxx);
        v_ini_parser_.ParseConfig("pedestrian_lidar", "voxelsize",
                                    pedestrian_lidar_params_.voxelsize);
        v_ini_parser_.ParseConfig("pedestrian_lidar", "roi_distance",
                                    pedestrian_lidar_params_.roi_distance);
        v_ini_parser_.ParseConfig("pedestrian_lidar", "offset",
...
double PedestrianLidar::GRTheta(double x, double y)
{
    double r;
    double theta;
...
void PedestrianLidar::VoxelizeData()
{
    pcl::VoxelGrid<pcl::PointXYZ> voxel_filter;
    float voxelsize = pedestrian_lidar_params_.voxelsize;
...
void PedestrianLidar::SetROI()
{
    ...
    for(unsigned int j=0; j<m_laser_cloud_in.points.size(); j++)
    {
        ...
        if(sqrt(pow(m_laser_cloud_in.points[j].x,2)+pow(m_laser_cloud_in.points[j].y,2)) > pedestrian_lidar_params_.roi_distance)
        {
            m_laser_cloud_in.points[j].x = -10.;
            m_laser_cloud_in.points[j].y = 0;
            m_laser_cloud_in.points[j].z = 0;
        ...
        {
            m_laser_cloud_in.points[j].y = m_laser_cloud_in.points[j].y + pedestrian_lidar_params_.offset;
        }
...
void PedestrianLidar::MakeTargetPoint()
{
    ...
    {
        if(temp_PointXYZ.points[j].x > pedestrian_lidar_params_.front_distance)
        {
...
void PedestrianLidar::UpdateRviz()
{
